...
#JVSコーパスのデータセットへのパス
jvs_dataset_path = "/src/datasets/jvs/jvs_ver1"
#txtファイル、サンプリングレート変換後のwavファイルの出力用ディレクトリ
output_dir = "/src/datasets/jvs/wav"
#出力するtxtファイル(学習用データセット用)の名前
output_train_txtfile_path = os.path.join("/src/datasets/jvs/", "jvs_train.txt")
#出力するtxtファイル(推論用データセット用)の名前
output_validation_txtfile_path = os.path.join("/src/datasets/jvs/", "jvs_val.txt")
#推論用データとして用いる音声ファイルの数
validation_file_number = 10
#出力する音声ファイルの出力先ディレクトリ
output_wav_dir = os.path.join(output_dir)
	...
	with open(transcripts_utf8_path) as f:
		for line in f:
			...
			if(os.path.exists(load_wav_file_path)):
				...
				#最後の句読点を削除
				text_converted = text_converted[:-1]
